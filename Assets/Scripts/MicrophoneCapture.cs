...
public class MicrophoneCapture : MonoBehaviour
{
    ...
    **/
    const string PROJECT_ID = "vspagent-icdq";
    const string email = "testvspagentserviceaccount@vspagent-icdq.iam.gserviceaccount.com";
    /* Make sure it is in the Assets Folder so that this is only the name */
    string certPath = "vspagent.p12";
...
/*    void OnGUI()
    ...
    public void StopListening()
    {
        if (recordingActive)
        {
            ...
            StartCoroutine(StartVoiceRequest("https://dialogflow.googleapis.com/v2/projects/" + PROJECT_ID + "/agent/sessions/" + sessionID + ":detectIntent",
                ACCESS_TOKEN,
                bytes));
        }
    }
    ...
    IEnumerator StartVoiceRequest(String url, String AccessToken, object parameter)
    {
        ...
        if (samples != null)
        {
            ...
            else
            {
                ...
                StartCoroutine(patient.WaitForEnd(audioClip));
